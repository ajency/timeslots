...
(function($) {
  var Timeline, addDescription, addTimeSlots, checkIfDatesBelongToSameDay, createDivWithClass, doesClassExists, getEndTime, getHourArray, getPixelPerSecond, getRandomColor, getStartTime, insertTimelineInterval, root, usedColors;
  root = this;
  usedColors = [];
  ...
  insertTimelineInterval = function($parent, intervalArray) {
    ...
    $parent.find('.timeline').append('<div class="overflow-timeline"> <div class="timeline-interval-region" style="height: 25px; position: relative;"></div> </div>');
  ...
  addTimeSlots = function($parent, timeArray) {
    var color, i, percentage, pixelPerSecond, slotTime, time, totalTime, width, _i, _len;
    ...
    $(".overflow-timeline").draggable({
      ...
      drag: function(e, ui) {
        ...
        if (positionLeft > 0) {
          console.log(positionLeft);
          ui.position.left = 0;
          return $(".overflow-timeline").css('left', '0px');
        }
      }
    });
