...
(function($) {
  var Timeline, addDescription, addSwipeEventHandlers, addTimeSlots, createChildRegion, getAllowedIntervalLabel, getCurrentTimestamp, getEndTime, getHourArray, getPixelPerSecond, getRandomColor, getStartTime, insertTimelineInterval, render, root, usedColors, validateSameDayTime, validateTimeArray;
  root = this;
  usedColors = [];
  ...
  addSwipeEventHandlers = function($parent, options) {
    var $timeslotDescription, distX, distY, startX, startY;
    ...
    return $timeslotDescription.on('swipeleft', function(e) {
      var isFirst, isLast, slotNo, template;
      if (!$(e.target).hasClass('slot')) {
        return;
      }
      slotNo = parseInt($(e.target).attr('data-slot'));
